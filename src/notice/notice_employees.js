/*
 * @flow
 * @providesModule notice_employees
 */
...
class notice_employees extends React.Component {
    state: State;
    ...
    render() {
        ...
        data_list.map(item=>{
            ...
            let datavaluelist=[];
            item.users.map(a=>{
                if(receiveData.selectItem.find(b=>b.selectedKey==a.uid)){
                    a={...a,checked:true};
                }
        ...
        return (<View flex>
            ...
            <SearchInput ref="search_box" backgroundColor={'transparent'} cancelTitle={YSI18n.get('Cancel')}
            ...
            /></View>
            <View style={{ height: YSWHs.height_window - 180 }}>
                <AlphabetListView
...
module.exports = connect(select, mapDispatchToProps)(notice_employees);
